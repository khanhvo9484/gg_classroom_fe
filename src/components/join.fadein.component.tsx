...
const FadeInJoin = ({onFadeClose}) => {
    ...
    const LoginSchema = Yup.object().shape({
        inviteCode: Yup.string()
          .required("Bạn cần nhập mã mời của lớp học"),
      });
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                const response = await customAxios.post("/courses/join-by-invite-code", payload);
        ...
        <Box>
            ...
            >
                <Container maxWidth="xl">
                    <Toolbar disableGutters>
                        ...
                        <Box
                            sx={{ flexGrow: 1, display: { xs: "none", md: "flex" }, gap: 1, color: "text.primary" }}
                        >
            ...
            >
                <Card sx={{ width: 600 }}>
                    ...
                    >
                        ...
                        <CardActions sx={{mt:3, mr: 1}}>
                            <Button size="small" variant='outlined'>Chuyển đổi tài khoản</Button>
                        </CardActions>
                    </Stack>
                </Card>
                <Card sx={{ width: 600 }}>
                ...
                <Box sx={{width:600, ml: 5}}>
                    ...
                    <ul>
                        <li>
                            <ListItemText sx={{ fontSize: 20, mb:1 }}
                                          secondary={"Sử dụng tài khoản được cấp phép"}/>
                        </li>
                        <li>
                            <ListItemText  sx={{ fontSize: 20, mb:1 }}
                                          secondary={"Sử dụng mã lớp học gồm 5-7 chữ cái hoặc số, không có dấu cách hoặc ký hiệu"}/>
                        </li>
                    </ul>
                    <Typography>
                        Nếu bạn đang gặp vấn đề khi tham gia lớp học, hãy chuyển đến bài viết trong Trung tâm trợ giúp
                    </Typography>
                </Box>
            </Box>
        </Box>
